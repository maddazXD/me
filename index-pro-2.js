...
(function () {
  ...
  /* ── 4. PWA: install prompt — banner auto + tombol ── */
  let deferredPrompt = null;

  // Inject banner DOM sekali
  const pwaBanner = document.createElement('div');
  pwaBanner.id = 'pwaBanner';
  pwaBanner.innerHTML = `
    <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
      <div style="width:40px;height:40px;border-radius:10px;background:#050914;border:1px solid rgba(0,255,179,.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-family:monospace;font-weight:700;color:#00ffb3;font-size:18px;">M</div>
      <div style="min-width:0;">
        <div style="font-weight:700;font-size:14px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Pasang MaddazXD</div>
        <div style="font-size:11px;color:rgba(255,255,255,.6);">Install sebagai aplikasi</div>
      </div>
    </div>
    <button id="pwaBannerInstall" style="padding:8px 16px;border-radius:8px;border:none;background:#00ffb3;color:#050914;font-weight:700;font-size:13px;cursor:pointer;flex-shrink:0;">Pasang</button>
    <button id="pwaBannerClose" style="background:none;border:none;color:rgba(255,255,255,.5);font-size:18px;cursor:pointer;padding:4px;flex-shrink:0;line-height:1;">✕</button>
  `;
  Object.assign(pwaBanner.style, {
    position:'fixed', bottom:'-100px', left:'50%', transform:'translateX(-50%)',
    width:'calc(100% - 32px)', maxWidth:'460px',
    background:'rgba(10,18,40,0.97)', border:'1px solid rgba(0,255,179,.25)',
    borderRadius:'16px', padding:'14px 16px', display:'flex',
    alignItems:'center', gap:'12px', zIndex:'99999',
  ...
  function showPwaBanner() {
    pwaBanner.style.bottom = '80px'; // di atas bottom nav
    // Auto tutup setelah 8 detik jika tidak direspons
    setTimeout(() => { if(pwaBanner.style.bottom !== '-100px') hidePwaBanner(); }, 8000);
  }
  function hidePwaBanner() {
    pwaBanner.style.bottom = '-100px';
  }
  ...
  document.getElementById('pwaBannerClose').onclick = hidePwaBanner;

  window.addEventListener("beforeinstallprompt", (e) => {
    e.preventDefault();
    deferredPrompt = e;
    // Tampilkan banner otomatis setelah 5 detik
    setTimeout(showPwaBanner, 5000);
    // Tampilkan juga tombol install di navbar
    const iBtn = $("#installBtn");
    if (iBtn) iBtn.style.display = "inline-flex";
  });

  // Tombol install di navbar/header (yang sudah ada)
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        /* FormSubmit AJAX: jangan pakai header Accept:application/json,
           cukup kirim FormData tanpa custom headers — FormSubmit akan return JSON otomatis */
        const r = await fetch(e.target.action, {
          method: "POST",
          body: fd,
        });
        /* FormSubmit kadang redirect 302 (first-time activation),
           anggap sukses jika status 200-299 atau 302 */
        if (r.ok || r.status === 302) {
          ...
          /* Jika FormSubmit belum diaktivasi, dia redirect ke halaman konfirmasi email */
          if (r.redirected || txt.includes("Confirm") || txt.includes("activate")) {
            origShowToast("⚠ Cek email untuk aktivasi FormSubmit pertama kali!");
          } else {
            origShowToast("Gagal mengirim pesan (" + r.status + ")");
          }
        }
      } catch (err) {
        origShowToast("Error jaringan, coba lagi.");
      }
      btn.disabled = false;
...
async function initMusicPlayer() {
  ...
  try {
    ...
    if (!res || !res.ok) {
      res = await fetch("list/playlist.json").catch(() => null);
    }
    if (res && res.ok) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ── 21. GAME LAUNCHER: fullscreen + auto landscape ── */
